...
### Политика удаления игрока — каскад

Удаление игрока в одной транзакции стирает его `rpeEntries` и вычищает id из
`rosterIds` всех сессий. Без каскада оценки-сироты были невидимы в результатах
сессий (джойн по живым игрокам), но продолжали учитываться в 7-day avg, а мёртвые
id раздували знаменатель `done/total` — две статистики расходились. Зомби-данных
в БД не существует в принципе; удаление подтверждается в UI (Yes/No).
