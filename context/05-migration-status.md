...
- [x] Кнопка «Did not train» в ScoreSheet — убирает игрока из состава сессии по ходу
      опроса, без режима Edit roster (вернуть игрока можно через Edit roster)
- [x] Все списки игроков отсортированы по имени (`byPlayerName` в shared/db) —
      survey, edit roster, settings, results/XLSX; results дополнительно сортируется по RPE
